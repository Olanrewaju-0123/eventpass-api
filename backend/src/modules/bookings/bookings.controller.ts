...
export class BookingsController {
   ...
   */
  static startBooking = asyncHandler(
   ...
   */
  static completeBooking = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
        const mapped = errors
   ...
   * Get booking by ID
   */
  static getBookingById = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const { bookingId } = req.params;
   ...
   */
  static getBookingByReference = asyncHandler(
    async (req: Request, res: Response) => {
      const { bookingReference } = req.params;

      const booking = await BookingsService.getBookingByReference(
        bookingReference
      );
  ...
  static getMyBookings = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      ...
      const { bookings, total } = await BookingsService.getUserBookings(
        req.user!.id,
        filters,
        {
          page,
   ...
   */
  static getAllBookings = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const page = Number.parseInt(req.query.page as string) || 1;
      const limit = Math.min(
        Number.parseInt(req.query.limit as string) || 10,
      ...
      const { bookings, total } = await BookingsService.getAllBookings(
        filters,
        {
          page,
          limit,
   ...
   */
  static getBookingStatistics = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
   ...
   */
  static getAllBookingStatistics = asyncHandler(
    async (req: AuthenticatedRequest, res: Response) => {
      const stats = await BookingsService.getBookingStatistics();

      return HttpResponse.success(
        res,
        "All booking statistics retrieved successfully",
        stats
      );
    }
  );
