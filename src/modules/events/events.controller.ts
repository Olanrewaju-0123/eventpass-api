...
import type { AuthenticatedRequest, CreateEventData, UpdateEventData } from "../../types"
...
export class EventsController {
  ...
  static createEvent = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (endDate <= startDate) {
      return HttpResponse.badRequest(res, "End date must be after start date")
    }
    ...
    return HttpResponse.success(res, "Event created successfully", event, 201)
  })
  ...
  static getEvents = asyncHandler(async (req: Request, res: Response) => {
    ...
    const limit = Math.min(Number.parseInt(req.query.limit as string) || 10, 50) // Max 50 per page
    ...
    return HttpResponse.paginated(res, events, page, limit, total, "Events retrieved successfully")
  })
  ...
  static updateEvent = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const fields = ["title", "description", "venue", "address", "category", "imageUrl"]
    ...
    if (req.body.price !== undefined) updateData.price = Number.parseFloat(req.body.price)
    if (req.body.capacity !== undefined) updateData.capacity = Number.parseInt(req.body.capacity)

    const event = await EventsService.updateEvent(eventId, updateData, req.user!.id)
  ...
  static getMyEvents = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const { events, total } = await EventsService.getEventsByCreator(req.user!.id, {
      page,
      limit,
      sortBy,
      sortOrder,
    ...
    return HttpResponse.paginated(res, events, page, limit, total, "Your events retrieved successfully")
  })

  /**
   * Generate event promotion QR code
   */
  static generatePromotionQR = asyncHandler(async (req: Request, res: Response) => {
    const { eventId } = req.params

    const result = await EventsService.generateEventPromotionQR(eventId)

    return HttpResponse.success(res, "Event promotion QR code generated successfully", result)
  })
  ...
  static getCategories = asyncHandler(async (req: Request, res: Response) => {
    ...
    return HttpResponse.success(res, "Event categories retrieved successfully", categories)
  })

  /**
   * Get event statistics
   */
  static getEventStatistics = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { eventId } = req.params

    const stats = await EventsService.getEventStatistics(eventId)

    return HttpResponse.success(res, "Event statistics retrieved successfully", stats)
  })
}
